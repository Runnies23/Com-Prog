...
class Task:
    ...
    Flippy Mozart's, Flippy's, and Turtle's home will randomly appear
    ANYWHERE on the map.
    """
    def start(self):
        stage.reset()
        ...
        if INTERACTIVE:
            ...
            shuffle(locations)
            (mozart_x,mozart_y),(flippy_x,flippy_y) = locations
        else:
        ...
        stage.add_object(Road(mozart_x,mozart_y,flippy_x,flippy_y,5,"yellow"))
        ...
        return stage.turtle,self.home,self.mozart,self.flippy

    def shortest_path(self):
        dist1 = ( distance(0,0,self.mozart.x,self.mozart.y) +
                  distance(self.mozart.x,self.mozart.y,self.flippy.x,self.flippy.y) +
                  distance(self.flippy.x,self.flippy.y,self.home.x,self.home.y) )
        dist2 = ( distance(0,0,self.flippy.x,self.flippy.y) +
                  distance(self.flippy.x,self.flippy.y,self.mozart.x,self.mozart.y) +
...
task = Task()
turtle,home,mozart,flippy = task.start()
mod.check = task.done
mod.turtle = turtle
mod.home = home
mod.mozart = mozart
